...
function getGN_Img( &$params, $link, $img, $pfx ) {
	...
	function getGN_List(&$params,$listCondition)
	{
		...
		foreach ( $rows as $row ) {
			...
			$articletime = strtotime($row->modified);
			$daynumber 	= $params->get( 'howmany', 5 );
//			echo $daynumber;
//			echo floor((time()-$articletime)/86400);
			if (floor((time()-$articletime)/86400) <= $daynumber ) {
			$lists[$i]->title .='<img style="border: 0pt none; vertical-align: middle;" alt="Just Added" title="" src="data/veritas/icon/new.gif"/>';
